...
const final_responses_path = `./codeAnalysis/final_responses/${parent_json_file_name}_final_responses.json`;
const ca_responses_path = `./codeAnalysis/ca_responses/${parent_json_file_name}_ca.json`;
...
const extractQuestionsData = (final_responses) => {
  ...
  let question_key_counter = 0; // Initialize the counter for question_key
  ...
  final_responses.forEach(final_response => {
    ...
    question_data["question_key"] = question_key_counter++; // Increment the counter for each question
  ...
  console.log("\nWriting into file\n");
  const jsonData = JSON.stringify(final_json_sheet, null, 2);
